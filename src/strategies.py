...
class MovingAverageCrossoverStrategy(Strategy):
    ...
    def __init__(self, params: Dict[str, Any]):
        ...
        self.short_window = int(params.get('short_window', 10)) # Default to 10
        self.long_window = int(params.get('long_window', 50))   # Default to 50
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'close' not in data.columns:
            raise ValueError("Dataframe must contain 'close' column for MA Crossover strategy.")

        signals_df = data.copy()
        signals_df['signal'] = 0.0 # Initialize signal column

        # Calculate moving averages
        signals_df['short_ma'] = signals_df['close'].rolling(window=self.short_window, min_periods=1).mean()
        signals_df['long_ma'] = signals_df['close'].rolling(window=self.long_window, min_periods=1).mean()
        ...
        print(f"Generated signals using MA Crossover (Short: {self.short_window}, Long: {self.long_window})")
        return signals_df
...
if __name__ == '__main__':
    ...
    if sample_data is not None:
        ...
        try:
            ...
            plt.plot(signals.index, signals['short_ma'], label=f'Short MA ({ma_params["short_window"]})', alpha=0.7)
            plt.plot(signals.index, signals['long_ma'], label=f'Long MA ({ma_params["long_window"]})', alpha=0.7)
