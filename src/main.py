...
import sys
import time
from typing import Any  # Import Dict and Any
...
from src.exchange import CoinbaseExchange, ExchangeInterface  # Add other exchanges here
from src.strategies import (  # Add other strategies here
...
def get_strategy_instance(config: dict[str, Any]) -> Strategy | None:
    ...
    if not StrategyClass:
        logging.error(
            f"Strategy class for '{enabled_strategy_name}' not found in STRATEGY_MAP."
        )
        return None
    ...
    try:
        ...
        logging.info(
            f"Instantiated strategy: {enabled_strategy_name} with params: {strategy_params}"
        )
...
def run_backtest(config: dict[str, Any]):
    ...
    # --- Fetch Data ---
    logging.info(
        f"Fetching data for {ticker} from {start_date} to {end_date} using {data_source}..."
    )
...
def run_live(config: dict[str, Any]):
    ...
    exchange_name = config.get(
        "exchange", "coinbase"
    )  # Assume default exchange if not specified
    ExchangeClass = EXCHANGE_MAP.get(exchange_name)
    if not ExchangeClass:
        logging.error(f"Exchange class for '{exchange_name}' not found.")
        return
    ...
    if not exchange.client:
        logging.error(
            "Cannot start live trading: Exchange client not initialized (check API keys)."
        )
        return
    ...
    trade_amount_base_sell = trading_cfg.get(
        "trade_amount_base_sell", "all"
    )  # 'all' or float
    ...
    def trading_job():
        ...
        try:
            ...
            logging.warning(
                "Using yfinance for recent data - Placeholder, NOT suitable for live trading!"
            )
            ...
            recent_data = fetch_historical_data(
                ticker=pair, start_date=None, end_date=None, interval="1h"
            )  # Placeholder!
            if recent_data is None or len(recent_data) < required_periods:
                logging.error(
                    f"Could not fetch sufficient recent data ({len(recent_data) if recent_data is not None else 0} < {required_periods}). Skipping job."
                )
                return
            ...
            logging.info(
                f"Latest Data Point: {latest_timestamp}, Price: {latest_price:.2f}, Signal: {latest_signal}"
            )
            ...
            logging.info(
                f"Balances - {quote_currency}: {quote_balance}, {base_currency}: {base_balance}"
            )
            ...
            currently_holding_base = (
                base_balance is not None and base_balance > 0.00001
            )  # Simple check
            ...
            if latest_signal == 1 and not currently_holding_base:  # Buy signal
                if quote_balance is not None and quote_balance >= trade_amount_quote:
                    logging.info(
                        f"BUY signal detected. Attempting to buy {trade_amount_quote} {quote_currency} worth of {base_currency}..."
                    )
                    order_result = exchange.place_buy_order(
                        pair, amount=trade_amount_quote, order_type="market"
                    )  # commit=False inside method
                    if order_result:
                        logging.info(
                            f"SIMULATED BUY order placed: {order_result.get('id', 'N/A')}"
                        )
                    ...
                    logging.warning(
                        f"BUY signal, but insufficient {quote_currency} balance ({quote_balance} < {trade_amount_quote}) or balance fetch failed."
                    )
                ...
                if base_balance is not None and base_balance > 0:
                    ...
                    if trade_amount_base_sell == "all":
                        ...
                        try:
                            ...
                            if sell_amount > base_balance:
                                logging.warning(
                                    f"Attempting to sell {sell_amount} {base_currency}, but only hold {base_balance}. Selling available amount."
                                )
                                sell_amount = base_balance
                        except ValueError:
                            logging.error(
                                f"Invalid float value for trade_amount_base_sell: {trade_amount_base_sell}"
                            )
                            return  # Skip sell

                    if sell_amount > 0:  # Ensure there's something to sell
                        logging.info(
                            f"SELL signal detected. Attempting to sell {sell_amount:.8f} {base_currency}..."
                        )
                        order_result = exchange.place_sell_order(
                            pair, amount=sell_amount, order_type="market"
                        )  # commit=False inside method
                        if order_result:
                            logging.info(
                                f"SIMULATED SELL order placed: {order_result.get('id', 'N/A')}"
                            )
                            # TODO: Update persistent position state
                        else:
                    ...
                    logging.warning(
                        f"SELL signal, but no {base_currency} balance ({base_balance}) or balance fetch failed."
                    )
            else:
