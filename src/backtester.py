...
from src.strategies import (  # Import specific strategies as needed
...
class Backtester:
    ...
    def __init__(
        self, config: dict[str, Any], strategy_instance: Strategy, data: pd.DataFrame
    ):
        """
        ...
        self.initial_capital = float(
            config.get("backtesting", {}).get("initial_capital", 10000.0)
        )
        self.trading_pair = config.get("trading", {}).get("pair", "UNKNOWN")
        self.results: pd.DataFrame | None = None  # Initialize results attribute

        if "close" not in self.data.columns or "signal" not in self.data.columns:
            raise ValueError(
                "Input data for Backtester must contain 'close' and 'signal' columns."
            )
    ...
    def run(self) -> pd.DataFrame:
        ...
        for index, row in self.data.iterrows():
            ...
            if pd.isna(row["signal"]):
                ...
                self.data.loc[index, "asset_value"] = (
                    current_position * row["close"] if not pd.isna(row["close"]) else 0
                )
                self.data.loc[index, "portfolio_value"] = (
                    cash + self.data.loc[index, "asset_value"]
                )
                continue
            ...
            if (
                ...
                and close_price > 0
            ):  # Simple: use all cash to buy
                current_position = cash / close_price
                cash = 0.0
                print(
                    f"{index.date()} BUY signal @ {close_price:.2f}. Holding: {current_position:.4f} units."
                )
            ...
            # Sell Signal (and currently long)
                ...
                print(
                    f"{index.date()} SELL signal @ {close_price:.2f}. Cash: ${cash:,.2f}"
                )

            # Update state for the current time step *after* potential trade
            asset_value = (
                current_position * close_price if not pd.isna(close_price) else 0
            )
    ...
    def calculate_performance(self) -> dict[str, Any]:
        ...
        daily_returns = self.results["returns"].iloc[
            1:
        ]  # Assuming first return is 0 or NaN
        ...
        max_drawdown_pct = (
            daily_drawdown.min() * 100 if not pd.isna(daily_drawdown.min()) else 0.0
        )
    ...
    def plot_results(self):
        ...
        self.results["portfolio_value"].plot(
            ax=ax1, label="Portfolio Value", color="blue"
        )
        ax1.set_title(
            f"{self.trading_pair} Backtest Performance ({self.strategy.__class__.__name__})"
        )
        ...
        self.results["close"].plot(
            ax=ax2, label="Close Price", color="black", alpha=0.8
        )
        ...
        ax2.plot(
            buy_signals.index,
            self.results.loc[buy_signals.index, "close"],
            "^",
            markersize=8,
            color="lime",
            lw=0,
        ...
        ax2.plot(
            sell_signals.index,
            self.results.loc[sell_signals.index, "close"],
            "v",
            markersize=8,
            color="red",
            lw=0,
    ...
    try:
        ...
        if data is not None:
            ...
            if not enabled_strategy_name or not strategy_params:
                print("No enabled strategy found in config. Exiting.")
                exit()
            ...
            #     strategy_instance = RsiStrategy(strategy_params)
                ...
                raise ValueError(
                    f"Unknown or unsupported strategy specified: {enabled_strategy_name}"
                )
