...
class ExchangeInterface:
    ...
    def place_buy_order(self, pair: str, amount: float, order_type: str) -> Optional[Dict[str, Any]]:
        """
        Places a buy order.
        Args:
    ...
    def place_sell_order(self, pair: str, amount: float, order_type: str) -> Optional[Dict[str, Any]]:
        """
        Places a sell order.
        Args:
            pair (str): Trading pair (e.g., BTC-USD).
    ...
    def get_historical_data(self, pair: str, granularity: str, start: Optional[str] = None, end: Optional[str] = None) -> Optional[pd.DataFrame]:
...
class CoinbaseExchange(ExchangeInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not api_key or not api_secret or api_key == 'YOUR_API_KEY':
            print("Warning: Coinbase API key/secret not configured. Live trading disabled.")
            self.client = None
        else:
            try:
                ...
                print(f"Successfully connected to Coinbase API for user: {user.get('name', 'N/A')}")
    ...
    def place_buy_order(self, pair: str, amount: float, order_type: str = 'market') -> Optional[Dict[str, Any]]:
        """
        ...
        if order_type.lower() != 'market':
             print("Warning: Basic Coinbase client only supports market-like buys via amount. Proceeding as market buy.")
        ...
        try:
            ...
            for acc in accounts.data:
                ...
                if acc['currency'] == quote_currency: # Use the wallet of the currency you're spending
                     account_id = acc['id']
                     break
            if not account_id:
            ...
            print(f"Attempting to place market BUY order for {amount} {quote_currency} worth of {pair.split('-')[0]}...")
            ...
            buy = self.client.buy(account_id,
                                  ...
                                  commit=False) # SET TO TRUE FOR ACTUAL EXECUTION
    ...
    def place_sell_order(self, pair: str, amount: float, order_type: str = 'market') -> Optional[Dict[str, Any]]:
        """
        ...
        Returns:
            Optional[Dict[str, Any]]: Dictionary representing the sell transaction, or None on failure.
        """
        if order_type.lower() != 'market':
             print("Warning: Basic Coinbase client only supports market-like sells via amount. Proceeding as market sell.")
        ...
        try:
            ...
            account_id = None
            for acc in accounts.data:
                if acc['currency'] == base_currency: # Use the wallet of the currency you're selling
                     account_id = acc['id']
                     break
            if not account_id:
            ...
            print(f"Attempting to place market SELL order for {amount} {base_currency}...")
    ...
    def get_historical_data(self, pair: str, granularity: str, start: Optional[str] = None, end: Optional[str] = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if granularity_sec not in valid_granularities:
             ...
             raise ValueError(f"Granularity must be an integer in seconds. Got: {granularity}")

        print(f"Fetching historical data for {pair} with granularity {granularity_sec}s (max 300 candles)...")
        try:
            ...
            df = pd.DataFrame(prices, columns=['time', 'low', 'high', 'open', 'close', 'volume'])
...
if __name__ == '__main__':
    try:
        ...
        if exchange.client: # Only proceed if client initialized
            ...
            if usd_balance is not None and usd_balance > 10: # Example: Buy $10 worth if balance > $10
            ...
            if base_balance is not None and base_balance > 0.0001: # Example: Sell 0.0001 units if available
            ...
            hist_data = exchange.get_historical_data(pair, granularity='3600') # Fetch most recent
